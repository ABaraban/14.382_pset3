...
setwd("/Users/abaraban/classes/metrics/14.382_pset3")
...
obj_names <- load("Mortgages.RData")     # returns object name(s) as character
mortgage  <- get(obj_names[1])           # the actual data.frame
...
pdf("LogitvsLinear1_mort.pdf", pointsize=15,width=6,height=6)
...
pdf("LogitvsProbit1_mort.pdf", pointsize=15,width=6,height=6)
...
spe.fit <- spe(fm = fmla1, data = mortgage, method = c("logit"), var = "black", us = us, alpha = alpha, b = R, parallel = TRUE, ncores = 24);
xtable(summary(spe.fit, result = "average"), digits=3);
pdf("Mortgage-SPE.pdf", pointsize=15,width=12,height=8)
plot(x = spe.fit, ylim = c(0, 0.25), ylab = "Change in Probability",main = " ",sub = "Logit Model")
dev.off();
...
spe.black.fit <- spe(fm = fmla1, data = mortgage, method = c("logit"), var = "black", us = us, subgroup = (mortgage[,"black"]==1), alpha = alpha, b = R, parallel = TRUE, ncores = 24);
xtable(summary(spe.black.fit, result = "average"), digits=3);
pdf("Mortgage-SPET.pdf", pointsize=14,width=12,height=8.0)
plot(x = spe.black.fit, ylim = c(0, 0.25), ylab = "Change in Probability",main = " ",sub = "Logit Model")
dev.off();
