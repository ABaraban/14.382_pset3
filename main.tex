...
We decided to reproduce the Gender Gap example. First, we show some summary statistics for the main variable of interest (log wage) together with covariates that will be used in the analysis. This shows the average size of the wage gap (in log points) and gives some idea of the distribution of covariates (e.g. fewer working women are married, more are college graduates or have advanced degrees).

\input{gender_gap_summary_stats}

Next, we show the raw log wage distributions for both genders:

\begin{figure}[h!]
    \centering
    \begin{subfigure}[c]{0.45\textwidth}
        \includegraphics{women_hist.png}
        \caption{Women's log wage distribution}
    \end{subfigure}
    \begin{subfigure}[c]{0.45\textwidth}
        \includegraphics{men_hist.png}
        \caption{Men's log wage distribution}
    \end{subfigure}
